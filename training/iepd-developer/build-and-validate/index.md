...
| --- | :---: | --- |
| MPD Catalog | Y | The MPD catalog is an XML file that identifies, locates, and classifies key artifacts in an IEPD. It does not have to list all of them. It may also include metadata such as points of contact<br><br>This file has the name "mpd-catalog.xml" and resides in the IEPD root directory. It contains a reference to the first XML catalog file in a catalog chain. |
| XML Catalog | Y | The XML catalog is an XML file that maps external entities to local ones (e.g., a target namespace to a local URI).<br><br>This file has the name "xml-catalog.xml." The IEPD can have multiple files with that name in different folders. One catalog file points to the next one in the catalog chain.
...
| Stylesheet | N | A stylesheet makes an XML instance and catalog human (or browser) readable. No universal guidance exists for the formatting or inclusion of XML stylesheets in IEPDs. Sample XML instances for an exchange should be linked to the stylesheet in order to view the instances in a consistent and presentable format. The MPD catalog should be linked to a stylesheet in order to view catalog metadata in a consistent and presentable format. |
